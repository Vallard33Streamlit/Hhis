...
if filter_v_imp_fr:
    ...
    l_filtres.append(("Importations européennes (en 1000$) supérieur à :", 10**log_value_v_imp_fr))


labels_sections, labels_sections_tree = load_labels()
# Filtre hs2
with st.sidebar.expander("Sélectionner les sections ou catégories HS2 à afficher", expanded=False, key="container_labels_sections_tree"):
    selected_categories_index = sac.tree(labels_sections_tree, open_index=[], index=list(labels_sections.index), checkbox=True, return_index = True, key="labels_sections_tree", height=500)
...
filter_by_taux_couverture = st.sidebar.checkbox("Filtrer les produits grâce à un taux de couverture", key="filter_by_taux_couverture")
if filter_by_taux_couverture:
    taux_couverture = st.sidebar.slider("Rapport entre les exportations et les importations inférieur à :", min_value=0.0, max_value=5.0, value=1.0, step=0.1, format="%.2f")
    df = df[df["Taux de couverture Exportations/Importations"] <= taux_couverture]
    l_filtres.append(("Rapport entre les exportations et les importations inférieur à :", taux_couverture))
...
if filter_by_taux_couverture_transit:
    rap_flux_transit = st.sidebar.slider("Rapport entre les flux transitants en Europe et les importations inférieur à :", min_value=0.0, max_value=5.0, value=1.0, step=0.1, format="%.2f")
    df = df[df["Taux de couverture de transit (Transit/Importations)"] <= rap_flux_transit]
    l_filtres.append(("Rapport entre les flux transitants en Europe et les importations inférieur à :", rap_flux_transit))
# Filtre taux_couverture_imp_exp
filter_by_taux_couverture_fr = st.sidebar.checkbox("Filtrer les produits grâce à un taux de couverture français", key="filter_by_taux_couverture_fr")
if filter_by_taux_couverture_fr:
    taux_couverture_fr = st.sidebar.slider("Rapport entre les exportations et les importations françaises inférieur à :", min_value=0.0, max_value=5.0, value=1.0, step=0.1, format="%.2f")
    df = df[df["Taux de couverture Exportations/Importations françaises"] <= taux_couverture_fr]
    l_filtres.append(("Rapport entre les exportations et les importations françaises inférieur à :", taux_couverture_fr))
...
if eq_trendeo_checkbox:
    ...
    l_filtres.append(("Garder uniquement les produits qui ont une nomenclature trendeo", True))
...
if filter_by_hhi_CDV:
    ...
    l_filtres.append(("HHI Contrôle opérationel supérieur à :", part_UE_CF))
# Filtre hhi_CF
